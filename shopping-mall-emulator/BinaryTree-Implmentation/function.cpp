	...
	char n2 = ((*leaf)->name).at(1);
	 ...
	 //if the incoming item name and current item name are the same
		...
		if( currentPtr != NULL){
			...
			currentPtr->num = currentPtr->num + count;
		}//end if
		...
		if(previousPtr1 == NULL){
			...
			previousPtr->nextListnode = newlist;
			newlist->nextListnode = currentPtr;
		}//end else
...
void Tree::printTree(treenode *leaf){
	...
	if (leaf != nullptrt){
		...
		cout <<" itemnum " << listPtr->num << "(" << listPtr->xpos << "," << listPtr->ypos << "," << listPtr->zpos<< ")";
